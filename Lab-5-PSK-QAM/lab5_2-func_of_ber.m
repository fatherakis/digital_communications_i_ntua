function errors=lab_Nyq_filter(k,Nsymb,nsamp,EbNo)
...
filtorder = delay*nsamp*2;
 rolloff = 0.33;
 % rtrapezium.m is in current working directory
...
% figure(1);pwelch(real(ytx),[],[],[],nsamp);% in 1/T scale
...
yrx = yrx(2*delay+(1:length(y))); % cinvolution edge trimming
%----------------------
yi=real(yrx); yq=imag(yrx); % in-phase and cross-phase conmponents
